...
class LstmLayer : public NeuronLayer {
 public:
  ~LstmLayer();
  void Setup(const LayerProto& proto, const vector<Layer*>& srclayers) override;
  void ComputeFeature(int flag, const vector<Layer*>& srclayers) override;
  ...
  Blob<float>* mutable_grad(const Layer* from) override {
    if (typeid(*from) == typeid(LstmLayer))
      return gradvec_[1];
    else
      return gradvec_[0];
  }
  ...
  const std::vector<Param*> GetParams() const override {
    ...
    if (bias_i_ != nullptr && bias_f_ != nullptr && bias_c_ != nullptr && bias_o_ !=nullptr) {
      ...
      params.push_back(bias_f_);
      params.push_back(bias_c_);
      params.push_back(bias_o_);
    }
    return params;
  }
 ...
 private:
  int batchsize_;  // batch size
  int vdim_, hdim_;  // dimensions
  Blob<float> *input_gate_, *forget_gate_, *new_memory_, *output_gate_,*memory_cell_;
  Param *weight_i_hx_, *weight_i_hh_, *bias_i_;  // input gate
  Param *weight_f_hx_, *weight_f_hh_, *bias_f_;  // forget gate
  Param *weight_c_hx_, *weight_c_hh_, *bias_c_;// new memory
  Param *weight_o_hx_, *weight_o_hh_, *bias_o_; // output gate
};
